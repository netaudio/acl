...
bool redis_string::setexnx(const char* key, const char* value,int timeout)
{
	return setexnx(key, strlen(key), value, strlen(value),timeout);
}

bool redis_string::setexnx(const char* key, size_t key_len,const char* value, size_t value_len,int timeout)
{
	const char* argv[6];
	size_t lens[6];
